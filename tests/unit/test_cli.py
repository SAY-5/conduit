"""CLI behaviour that needs no AWS: mapping rejection at submit and config output."""
...
def test_submit_rejects_invalid_tasks_up_front(tmp_path: Path, monkeypatch):
    ...
    result = CliRunner().invoke(
        app,
        ["submit", str(tasks), "-c", "webhook-crm", "--connectors-dir", str(ROOT / "connectors")],
    )
    ...
    assert "rejected BAD-2 v1: state:" in result.output and "(enum)" in result.output
    assert "submitted 1 messages (1 tasks x 1) to conduit-webhook-crm, 2 rejected" in result.output


def test_config_validate_reports_mapping_size():
    result = CliRunner().invoke(
        app, ["config", "validate", "--connectors-dir", str(ROOT / "connectors")]
    )
